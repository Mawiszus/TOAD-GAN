...
class Block2Vec(pl.LightningModule):
    def __init__(self, **kwargs):
        ...
        self.dataset = Block2VecDataset(
            self.args.input_world_path, coords=self.args.input_world_coords, cutout_coords=self.args.cutout_coords, neighbor_radius=self.args.neighbor_radius)
    ...
    def read_texture(self, block: str):
        if block not in self.textures and block != "air":
            texture_candidates = Path("/home/schubert/projects/TOAD-GAN/minecraft/block2vec/textures").glob("*.png")
            # use of absolute path is intentional
            match = process.extractOne(
                block, texture_candidates)
    ...
    def save_embedding(self, id2block: Dict[int, str], output_path: str):
        embeddings = self.model.target_embeddings.weight.cpu().data.numpy()
        embedding_dict = {}
    ...
    def plot_embeddings(self, embedding_dict: Dict[str, np.ndarray], output_path: str):
        ...
        ia = ImageAnnotations3D(
            list(embedding_dict.values()), texture_imgs, legend, ax, fig)
        plt.tight_layout()
        ...
        for i, b1 in id2block.items():
            ...
            for j, b2 in id2block.items():
                dists[i, j] = F.mse_loss(
                    self.model.target_embeddings.weight.data[i], self.model.target_embeddings.weight.data[j])
